#!/bin/bash

# Script to download the MERL-MIT BRDF database.

URL='http://people.csail.mit.edu/wojciech/BRDFDatabase/brdfs'  # URL to download from.
DL_TO_DIR='./brdf_data/'  # Directory to download to.

files=(
    "alumina-oxide.binary"
    "aluminium.binary"
    "aventurnine.binary"
    "beige-fabric.binary"
    "black-fabric.binary"
    "black-obsidian.binary"
    "black-oxidized-steel.binary"
    "black-phenolic.binary"
    "black-soft-plastic.binary"
    "blue-acrylic.binary"
    "blue-fabric.binary"
    "blue-metallic-paint.binary"
    "blue-metallic-paint2.binary"
    "blue-rubber.binary"
    "brass.binary"
    "cherry-235.binary"
    "chrome-steel.binary"
    "chrome.binary"
    "colonial-maple-223.binary"
    "color-changing-paint1.binary"
    "color-changing-paint2.binary"
    "color-changing-paint3.binary"
    "dark-blue-paint.binary"
    "dark-red-paint.binary"
    "dark-specular-fabric.binary"
    "delrin.binary"
    "fruitwood-241.binary"
    "gold-metallic-paint.binary"
    "gold-metallic-paint2.binary"
    "gold-metallic-paint3.binary"
    "gold-paint.binary"
    "gray-plastic.binary"
    "grease-covered-steel.binary"
    "green-acrylic.binary"
    "green-fabric.binary"
    "green-latex.binary"
    "green-metallic-paint.binary"
    "green-metallic-paint2.binary"
    "green-plastic.binary"
    "hematite.binary"
    "ipswich-pine-221.binary"
    "light-brown-fabric.binary"
    "light-red-paint.binary"
    "maroon-plastic.binary"
    "natural-209.binary"
    "neoprene-rubber.binary"
    "nickel.binary"
    "nylon.binary"
    "orange-paint.binary"
    "pearl-paint.binary"
    "pickled-oak-260.binary"
    "pink-fabric.binary"
    "pink-fabric2.binary"
    "pink-felt.binary"
    "pink-jasper.binary"
    "pink-plastic.binary"
    "polyethylene.binary"
    "polyurethane-foam.binary"
    "pure-rubber.binary"
    "purple-paint.binary"
    "pvc.binary"
    "red-fabric.binary"
    "red-fabric2.binary"
    "red-metallic-paint.binary"
    "red-phenolic.binary"
    "red-plastic.binary"
    "red-specular-plastic.binary"
    "silicon-nitrade.binary"
    "silver-metallic-paint.binary"
    "silver-metallic-paint2.binary"
    "silver-paint.binary"
    "special-walnut-224.binary"
    "specular-black-phenolic.binary"
    "specular-blue-phenolic.binary"
    "specular-green-phenolic.binary"
    "specular-maroon-phenolic.binary"
    "specular-orange-phenolic.binary"
    "specular-red-phenolic.binary"
    "specular-violet-phenolic.binary"
    "specular-white-phenolic.binary"
    "specular-yellow-phenolic.binary"
    "ss440.binary"
    "steel.binary"
    "teflon.binary"
    "tungsten-carbide.binary"
    "two-layer-gold.binary"
    "two-layer-silver.binary"
    "violet-acrylic.binary"
    "violet-rubber.binary"
    "white-acrylic.binary"
    "white-diffuse-bball.binary"
    "white-fabric.binary"
    "white-fabric2.binary"
    "white-marble.binary"
    "white-paint.binary"
    "yellow-matte-plastic.binary"
    "yellow-paint.binary"
    "yellow-phenolic.binary"
    "yellow-plastic.binary"
)

for curr in "${files[@]}"
do
    local_file="${DL_TO_DIR}/${curr}"
    if [ -e "${local_file}" ]
    then
        echo "File ${local_file} already exists.  No need to download."
    else
        echo "Downloading file: ${curr}..."
        wget "$URL/${curr}" -O "$local_file"
    fi
done

